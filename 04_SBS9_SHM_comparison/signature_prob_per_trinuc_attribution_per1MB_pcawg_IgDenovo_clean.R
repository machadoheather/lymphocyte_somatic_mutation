...
files <- list.files(paste(root, "/lymphocyteWGS/mutsig_byregion/pcawg_IgDenovo/", cancertype, "/", sep=""), pattern = paste("exposures_", cancertype, ".sigfit_union_hdp_sigprofiler_cosmic3_IgDenovo", sep="") )
inlist = list()
infolist = list()
for (i in 1:length(files)){
  load(paste(root, "/lymphocyteWGS/mutsig_byregion/pcawg_IgDenovo/", cancertype, "/", files[i], sep=""))
...
focalsample = read.table(paste(root, "/pcawg/chr_pos_ref_alt.", cancertype, ".txt", sep=""), stringsAsFactors = F, header=T)
...
write.table(my_attribution_perbp, paste(root, "/lymphocyteWGS/mutsig_byregion/pcawg_IgDenovo/attribution_perbp_sigfit_", cancertype, "_1Mb_min10percent_IgDenovo.txt", sep=""), col.names = T, row.names = F, quote=F, sep="\t")
