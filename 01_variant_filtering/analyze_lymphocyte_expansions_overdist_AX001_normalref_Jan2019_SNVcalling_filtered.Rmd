---
title: "AX001 SNV filter final July, 2019"
...
knitr::opts_chunk$set(cache=TRUE, cache.lazy = FALSE, fig.path="./graphics/plot",autodep=TRUE,fig.width=5, fig.height=4, cache.lazy = FALSE)
...
shearw = read.table(paste("../AX001/AX001.shearwater.vaf40.txt", sep=""), sep="\t", stringsAsFactors = F, header=T)
...
HQdp1 = read.table(paste("/Users/hm8/sanger/lymphocyteExpansionSequenceAnalysis/AX001/AX001_totalDP.txt", sep=""), stringsAsFactors = F, header=T, check.names=F)[,metadata$colony]
HQalt1 = read.table(paste("/Users/hm8/sanger/lymphocyteExpansionSequenceAnalysis/AX001/AX001_alternateDP.txt", sep=""), stringsAsFactors = F, header=T, check.names=F)[,metadata$colony]
...
# Now fine: 
# removed due to no variant calling (showed up on polyclonal test): "T8_B5"  "T8_B7"  "T8_E11" "T8_E8"  "T8_F11" "T9_B5"
# removed due to no variant calling (0 variants passed): T8_A12 T8_A4  T8_A9  T8_B10 T8_B9  T9_A12
#dpkeep = !(sample_names1 %in% c("T8_B5",  "T8_B7",  "T8_E11", "T8_E8",  "T8_F11", "T9_B5","T1_A3", "T8_A12", "T8_A4",  "T8_A9",  "T8_B10", "T8_B9",  "T9_A12", "B11_G2") )

dpkeep = !(sample_names1 %in% c("T1_A3", "B11_G2") )
...
bb_df = na.omit(read.table(paste("../AX001/AX001_rho.txt", sep=""), stringsAsFactors = FALSE, header=TRUE))
...
HQalt_keepA = subset(HQalt, !(rownames(HQalt) %in% flt_rho_toss) )
HQfreq_keepA = subset(HQfreq, !(rownames(HQfreq) %in% flt_rho_toss) )
rm(shearwA40, HQdp40, HQdp1, HQalt1, HQfreq1, bb_df, rho_est, pvalue)

```

## Recovering Lee-Six variants (high frequency variants) lost during filtering
```{r, message=FALSE, warning=FALSE, fig.width=2, fig.height=2}
oldsnv = read.table("Shearwater_calls_FDR0.95_all_muts.txt", header=TRUE, stringsAsFactors = F)
oldsnv$ID = paste(oldsnv$X.CHROM, ":", oldsnv$POS, "_", oldsnv$REF, "/", oldsnv$ALT, sep="")  
pickup = oldsnv[oldsnv$counts >= 14,c("ID","X.CHROM","POS","REF","ALT", "counts")] # 40 sites

# # Are they in the initial files?
# sum(pickup$ID %in% shearwA40$ID) # only 15 passed initial shearwater filtering (and vaf40 cutoff)- NOT USED
# sum(pickup$ID %in% rownames(HQdp1)) # 39 (all but one) found in Caveman (and fetched with cgpvaf)
# sum(pickup$ID %in% rownames(HQdp40)) # 39 pass VAF filtering
# sum(pickup$ID %in% flt_rho_keep) # all 39 pass rho filtering
# sum(pickup$ID %in% rownames(HQdp) ) # 32 passed shearwater p-value filtering
# sum(pickup$ID %in% rownames(HQalt_keepA)) # 32 kept 
to_pickup = pickup$ID[!(pickup$ID %in% rownames(HQalt_keepA) )]


# Recover and merge with rest of variants
HQalt_keepB = subset(HQalt40, (rownames(HQalt40) %in% to_pickup) )[,dpkeep]
HQfreq_keepB = subset(HQfreq40, (rownames(HQfreq40) %in% to_pickup) )[,dpkeep]
HQalt_keep = rbind(HQalt_keepA, HQalt_keepB)
HQfreq_keep = rbind(HQfreq_keepA, HQfreq_keepB)
rm( HQalt40 , HQfreq40,  HQalt_keepA, HQalt_keepB, HQfreq_keepB, HQfreq_keepA, HQalt, HQfreq, oldsnv)
```
...
rm(HQfreqsomNA, m1, m2, m3, HQfreq_keep_non0, tmp, meanVAF_alt2_keep, Nsamples_alt2_keep, meanVAF_alt2_freq20_keep,Nsamples_alt2_freq20_keep)
```
...
table(m1$Cell.type2)
# B Memory  B Naive      HSC T Memory  T Naive     Treg 
#       21       46       43       34      125       11 
```
...
rm(gens, gens_non0, HQfreqsom3, HQfreqsom3b )
```
...
m2$Cell.type2 = factor(m2$Cell.type2, levels=c("HSC","B Naive","B Memory","T Naive","T Memory","Treg") ) 
m3 = merge(data.frame(colony=names(meandp), meandp), m2, by=1)
colorCodes=c("HSC"="#238b45", "B Naive"="#dd3497", "B Memory"="#7a0177", "T Naive"="#41b6c4", "T Memory"="#253494", "Treg"="grey")
...
## Normalize all to a read depth of 30:
```{r}
new_nmut = function(rd, nmut){
  nmut + ( mymodel(30) - mymodel(rd))
}
...
```{r}
Nmut_med = round(tapply(m3$Nmut_adj_as, INDEX=m3$Cell.type2 , FUN=median))
ggplot(m3) +
