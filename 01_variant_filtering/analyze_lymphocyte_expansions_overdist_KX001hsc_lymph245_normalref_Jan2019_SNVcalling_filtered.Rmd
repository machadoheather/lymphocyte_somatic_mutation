---
title: "KX001hsc_lymph245_filtered SNV filter final July, 2019"
...
knitr::opts_knit$set(root.dir = '/Users/hm8/sanger/lymphocyteExpansionSequenceAnalysis/KX001hsc_lymph245_filtered')
library(ggplot2)
library(cowplot)
library(caTools)
library(reshape2)
...
# removed due to low dp: "PD40667up"
# removed due to polyclonal: "PD40521e" "PD40521g", possibly "PD40521d"
# removed due to aberant (few) mut counts, possibly contamination: "PD40521c", "PD40521d"
# removed due to not present in shearwater: PD40521n
hsc_omit = c("PD40521hs", "PD40521id", "PD40521il", "PD40521io", "PD40521ke", "PD40521mj", "PD40521mx", "PD40521ng", "PD40521y", "PD40521bh")
dpkeep = !(sample_names %in% hsc_omit)  & !(sample_names %in% c("PD40521e", "PD40521g","PD40521n", "PD40667up", "PD40521c", "PD40521d") )
...
m2$Cell.type3 = m2$Cell.type2
m2$Cell.type3[ !(m2$Cell.type3 %in% c("HSC","B Naive","B Memory","T Naive","T Memory" ))] = "other"
m2$Cell.type3 = factor(m2$Cell.type3, levels=c("HSC","B Naive","B Memory","T Naive","T Memory","other") ) 
m3 = merge(data.frame(colony=names(meandp), meandp), m2, by=1)
colorCodes=c("HSC"="#238b45", "B Naive"="#dd3497", "B Memory"="#7a0177", "T Naive"="#41b6c4", "T Memory"="#253494", "other"="grey")
ggplot(m3) +
  geom_point(aes(meandp,Nmut,fill=Cell.type3), pch=21) + 
...
```{r}
### Asymptotic model
#m4 = m3[m3$Cell.type3 == "HSC" ,  ]
m4 = m3[ (m3$Cell.type3 == "B Naive" | m3$Cell.type3 == "T Naive") & m3$meandp<50 ,  ] ## at very high dp, get errors
...
ggplot(m3) +
  geom_point(aes(meandp,Nmut_adj_as,fill=Cell.type3), pch=21) + 
  scale_fill_manual(values=colorCodes) + 
  ylab("# Mutations") +
  xlab("Coverage per colony")+
  ggtitle(groupname)
...
```{r}
Nmut_med = round(tapply(m3$Nmut_adj_as, INDEX=m3$Cell.type3 , FUN=median))
ggplot(m3) +
  geom_boxplot(aes(Cell.type3,Nmut_adj_as,fill=Cell.type3)) + 
  scale_fill_manual(values=colorCodes) + 
  ylab("# Mutations") +
  xlab("Cell type")+
...
```{r, message=FALSE, warning=FALSE}
celltypes = levels(factor(m3$Cell.type3) )
mutcounts_matrix_celltypes = matrix(ncol=length(celltypes), nrow=96)
for (i in 1:length(celltypes)){
  mat1 = mutcounts_matrix[,which(names(somsample_list2) %in% m3$colony[m3$Cell.type3==celltypes[i]])]
