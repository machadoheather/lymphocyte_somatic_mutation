---
title: "CB001 indel filtering July 2019"
...
knitr::opts_knit$set(root.dir = '/Users/hm8/sanger/lymphocyteExpansionSequenceAnalysis/CB001_filtered')
library(ggplot2)
library(cowplot)
library(caTools)
library(reshape2)
...
HQdp1 = read.table(paste("../stemcellCB2hsc_lymph22/stemcellCB2hsc_lymph22_indel_totalDP.txt", sep=""), stringsAsFactors = F, header=T, check.names=F)
HQalt1 = read.table(paste("../stemcellCB2hsc_lymph22/stemcellCB2hsc_lymph22_indel_alternateDP.txt", sep=""), stringsAsFactors = F, header=T, check.names=F)
...
# removed due to low dp: 
# removed due to polyclonal: PD42874b_lo0010"
dpkeep = !(sample_names %in% "PD42874b_lo0010")
...
```{r}
bb_df = na.omit(read.table(paste("../stemcellCB2hsc_lymph22/stemcellCB2hsc_lymph22_indel_rho.txt", sep=""), stringsAsFactors = FALSE, header=TRUE))
...
```{r}
## Asymptotic model
m4 = m3[m3$Cell.type2 == "B Naive" | m3$Cell.type2 == "T Naive",  ]
#m4 = m3[m3$Cell.type2 == "B Naive" | m3$Cell.type2 == "T Naive" | m3$Cell.type2 == "HSC",  ]
#m4 = m3
#m4 = m3[m3$Cell.type2 == "T Naive",  ]
...
#nls(data.frame(m4sorted) ~  b0 + b1*(1-exp(-exp(lrc) * x)) )
nlsfitSS <- nls(y ~ SSasymp(x, Asym, R0, lrc),data=m4sorted, control=nls.control(minFactor = 1/10024, warnOnly = TRUE,maxiter = 500))
nlsfitSS
...
ggplot(subset(m3, Cell.type2 %in% c("HSC","B Naive","B Memory","T Naive","T Memory","Treg") ), aes(Nmut_adj_as_indel,Nmut_adj_as, fill=Cell.type2) )+
  geom_point(pch=21)+
